...
class ousb {
    public static void main(String[] args) throws IOException {
        ...
        while(true){
            ...
            if (!option.equals("4") & cmd.length() > 1){ //Input handling for different ousb interface options making sure explicit values.
                System.out.println("Invalid command argument.");
                System.out.println("Available Commands: " + help);
            }
            
            if (!option.equals("0") & !option.equals("1")){
                ousb = checkousb(base + " io pinc"); //check if ousb connected
            ...
            // Use switch statement to handle different OUSB functionality
            switch(option){
                ...
                case "4": // Write to portb
                    ...
                    if(cmd.length() == 1){ //event that no 2nd argument given
    ...
    public static boolean checkousb(String ousbcmd) throws IOException {
        try {
            Runtime run = Runtime.getRuntime(); // get runtime shell.
            Process pr = run.exec(ousbcmd);         // start command.
            pr.waitFor();                       // wait for command to finish.
